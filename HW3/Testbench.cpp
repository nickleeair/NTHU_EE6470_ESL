...
void Testbench::fetch_result() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = ir_result.get();
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = ig_result.get();
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = ib_result.get();
