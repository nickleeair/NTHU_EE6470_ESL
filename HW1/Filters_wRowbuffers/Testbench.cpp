...
void Testbench::do_filter()
{
  ...
  for (y = -1; y != height + 1; ++y) // consider the zero padding
  {
    for (x = -1; x != width + 1; ++x) // consider the zero padding
    {
      /*std::cout << "x:  " << x << " / "
                << "y:  " << y << std::endl;
      std::cout << sc_core::sc_time_stamp() << endl;*/
    ...
    {
      if (ir_result.num_available() == 0)
        wait(ir_result.data_written_event());
      // std::cout << "wait OK! " << std::endl;
      for (i = 0; i < 510; i++)
      {
        *(target_bitmap + bytes_per_pixel * (width * y + i) + 2) = ir_result.read();
        *(target_bitmap + bytes_per_pixel * (width * y + i) + 1) = ig_result.read();
        *(target_bitmap + bytes_per_pixel * (width * y + i) + 0) = ib_result.read();
        wait(1);
      }
    }
  }
