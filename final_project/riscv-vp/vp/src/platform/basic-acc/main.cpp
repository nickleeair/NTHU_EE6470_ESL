...
int sc_main(int argc, char **argv)
{
	...
	CombinedMemoryInterface iss_core0_mem_if("MemoryInterface0", core0);
	CombinedMemoryInterface iss_core1_mem_if("MemoryInterface1", core1);
	...
	iss_core0_mem_if.bus_lock = bus_lock;
	iss_core1_mem_if.bus_lock = bus_lock;

	instr_memory_if *instr_core0_mem_if = &iss_core0_mem_if;
	data_memory_if *data_core0_mem_if = &iss_core0_mem_if;

	if (opt.use_instr_dmi)
		instr_core0_mem_if = &instr_mem_0;
	if (opt.use_data_dmi)
	{
		iss_core0_mem_if.dmi_ranges.emplace_back(dmi);
	}

	instr_memory_if *instr_core1_mem_if = &iss_core1_mem_if;
	data_memory_if *data_core1_mem_if = &iss_core1_mem_if;

	if (opt.use_instr_dmi)
		instr_core1_mem_if = &instr_mem_1;
	if (opt.use_data_dmi)
	{
		iss_core1_mem_if.dmi_ranges.emplace_back(dmi);
	}
	...
	core0.init(instr_core0_mem_if, data_core0_mem_if, &clint, entry_point, opt.mem_end_addr - 3);
	core1.init(instr_core1_mem_if, data_core1_mem_if, &clint, entry_point, opt.mem_end_addr - 32767);
	// core2.init(instr_core2_mem_if, data_core2_mem_if, &clint, entry_point, opt.mem_end_addr - 3);
	...
	// connect TLM sockets
	iss_core0_mem_if.isock.bind(bus.tsocks[0]);
	iss_core1_mem_if.isock.bind(bus.tsocks[1]);
	...
	sc_core::sc_start();
	std::cout << "test time == " << sc_core::sc_time_stamp() << std::endl;
	if (!opt.quiet)
	{
		core0.show();
		core1.show();
